...
<Note>
`from_table` and `to_table` are **bare relation names** — `articles`, not
`public.articles`. Postgres reports the schema separately, so a
schema-qualified value matches nothing and the spec silently produces no
edges. An edge `name` must also not collide with a table you replicate:
the engine rejects that at startup, because edge rows written into a
document table would overwrite documents that share an id.
</Note>
...
- **FKs must target the referenced table's primary key** — edge
  endpoints are built from the FK values as `to_table` record ids, and
  those ids are PK-derived. FKs onto non-PK unique columns won't
  resolve.
