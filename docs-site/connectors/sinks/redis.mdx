...
```text
ventstream:orders:production:{orders}:shop.orders%3A%5B%22ord-42%22%5D
```
...
truncate, or rebuild. Deletes remove the key immediately.
...
Give each deployment revision a stable writer identity. Managed deployments use
...
In managed mode, a restarted managed engine keeps its pipeline stopped until
it acquires the current control session and receives convergence work.
An already-running engine may continue through a temporary control-plane
outage; if VentStream Cloud later admits a replacement, Redis fencing prevents
the older process from overwriting the replacement.
