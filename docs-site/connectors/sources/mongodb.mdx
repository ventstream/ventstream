...
sink. In v1 each MongoDB document maps to **one** sink document (raw 1:1) —
which fits MongoDB well, since documents are usually already denormalized.
Cross-collection joins (embedding a referenced collection) are on the
...
- **Raw 1:1 only.** One document → one sink document; there are no
  ...
  not recompose a parent document. (Phase 2: cross-collection joins via the
  shared join engine.)
