...
In the [demo](/quickstart) the engine connects as `neo4j` — the default
...
Neo4j CDC cursor lives on the **engine's PVC**, not the server — retiring
an engine leaves nothing behind on Neo4j. The flip side: if an engine is
...
re-bootstrap. Size retention to cover your worst-case engine downtime.
</Note>
...
`VS_NEO4J_TRUST_CERT_FILE` is an older alias for the same CA bundle — configure
only one of the two; setting both is a startup error.
...
VS_NEO4J_TLS_CA_FILE=/etc/ventstream/neo4j-ca.crt \
