...
unwraps each message into a sink document. In v1 each message maps to **one**
sink document (raw 1:1), keyed by the row's primary key. Cross-topic joins
(denormalizing several Debezium topics into one document) are on the
[roadmap](#limitations-v1).
...
supported in v1 — it needs an AWS SigV4 token provider (planned).
...
- **Raw 1:1 only.** One message → one sink document; no cross-topic joins yet,
  so a change on a *referenced* table's topic doesn't recompose a parent
  document. (Phase 2: cross-topic joins via the shared join engine — this is
  what makes the Kafka source worth more than a stock Kafka→ES sink connector.)
...
- **Schema changes aren't tracked.** A Debezium DDL/schema-change event carries
  ...
  (OpenSearch dynamic mapping permitting). Explicit schema-evolution handling is
  planned for v2.
...
- **MSK IAM / MSK Serverless** need an AWS SigV4 token provider — not in v1.
...
- **Offsets commit on a single global prefix.** Offsets advance only through
  ...
  Conservative and safe (never commits past un-written data); just less
  granular under a multi-partition stall.
