...
VS_MYSQL_TLS_CA_FILE=/run/secrets/mysql-ca.pem
```

The CA file is optional when the server certificate chains to a public root.
Strict mode validates both the certificate chain and hostname for binlog,
snapshot, join-fetch, and SQL recomposition connections.
...
| `VS_MYSQL_TLS_CA_FILE` | unset | PEM CA bundle for a private CA. |
