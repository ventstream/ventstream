...
composite keys) are shared with the [Postgres connector](/connectors/sources/postgres).
...
| Memory | **Scales with the joined working set** (~1 KiB/row; can OOM at large scale) | **Bounded — O(chunk)**, independent of dataset (measured ~77 MiB for 100k orders / 200k items / 5k customers) |
...
- **`ROW` binlog format required** — statement/mixed formats carry no per-row
  images. The source tails **positions, not GTIDs**, in v1: position resume is
  robust on a single server, but a failover to a different primary is recovered
  by re-bootstrap rather than seamless reposition. GTID resume is a v2
  robustness play.
...
- **Binlog-bounded resume.** A resume reaches back only as far as the binlog is
  retained (`binlog_expire_logs_seconds`); past that the source re-bootstraps
  automatically (see [How it works](#how-it-works)).
