...
3. **The project CA certificate** — downloaded from the database settings
   page, for TLS verification (details below).
4. **A publication** listing the tables you want to stream (details below).
...
source:
  ...
  postgres:
    ...
    tls:
      mode: verify_full
      ca_file: /etc/ventstream/supabase-ca.crt
```

Supabase signs its database certificates with its own CA ("Supabase Root
2021 CA") — it is not in system trust stores, so `verify_full` needs the
project CA certificate: download it from the database settings page and
point `tls.ca_file` at it.
