...
The Postgres source reads logical replication and streams each change into the
sink — as flat per-row documents by default, or with related rows embedded into
each primary's document when you add a joins spec. This guide uses the same
`shop.orders` example as the [quickstart](/quickstart): each order document
carries its customer (1:1) and its line items (1:many).

<Note>
VentStream writes to a **target**:
[OpenSearch / Elasticsearch](/connectors/sinks/opensearch) (one connector —
they share the `_bulk` API),
[Meilisearch](/connectors/sinks/meilisearch),
[Redis](/connectors/sinks/redis), or
[SurrealDB](/connectors/sinks/surrealdb). The examples here use OpenSearch;
set the endpoint with `VS_OS_ENDPOINT`.
</Note>

## Flat sync — no spec required

With no joins spec, every published table streams as-is: each row becomes one
document with a deterministic id built from the fully-qualified table name and
the replica-identity key columns (the primary key by default) —
`shop.orders:["ord-0001"]`. The snapshot bootstrap derives the same id from the
discovered primary key. Updates overwrite the document in place, deletes always
find and remove it, and an update that changes the primary key removes the old
document and writes the new one. This holds across all sinks. A minimal,
complete config:
...
The table scope is the Postgres **publication** — whatever it lists is what
streams. Add a `joins:` spec (below) only when you want **composed** documents:
a parent with embedded children.

<Warning>
Two replica-identity edge cases:

- `REPLICA IDENTITY NOTHING` blocks `UPDATE`/`DELETE` **at the database
  level** on a published table — Postgres rejects the statement itself, so
  don't set it. The default replica identity is all the engine needs.
- A **PK-less** table streams inserts without a stable id — documents won't
  upsert (each insert is a new document) — and the snapshot **skips** it with
  a warning. Give tables a primary key (or a replica-identity unique index).
</Warning>

## Production prerequisites

In the [demo](/quickstart) the engine appears to "just work" with zero
...
<AccordionGroup>
  <Accordion title="RDS / Aurora PostgreSQL" icon="aws">
    ...
    **3. Create the publication** as the table owner (or `rds_superuser`).
    ...
    engine either resumes from its slot or performs the expected controlled
...
<Note>
**Decommissioning an engine:** a slot the engine created keeps pinning WAL
until it's dropped — Postgres won't recycle that WAL, and the disk
eventually fills. When you retire an engine, drop its slot:
`SELECT pg_drop_replication_slot('ventstream_orders_slot');`
</Note>

## WAL retention and slot health

A logical slot pins WAL until the engine confirms it — that's what makes
restarts lossless. Three things keep it healthy:

- **Idle databases advance too.** The engine acknowledges the server's
  keepalives whenever everything it has emitted is durably written, so
  a quiet publication (or traffic only on unpublished tables) doesn't
  pin WAL behind the slot.
- **Watch `vs_pg_slot_lag_bytes`.** The engine polls
  `pg_replication_slots` every 30 seconds and exports retained-WAL lag
  as a gauge. Sustained growth means the sink can't keep up or the
  engine is down.
- **Set a server-side backstop.** `max_slot_wal_keep_size` (e.g.
  `10GB`) caps how much WAL any slot may retain. If the cap is hit the
  slot is invalidated and the engine re-bootstraps — a full resync, but
  the database never fills its disk.

## Partitioned tables

Publish partitioned tables **through the root**:

```sql
ALTER PUBLICATION vs_pub SET (publish_via_partition_root = true);
```

Without it, leaf partitions decode as separate relations — projections
written against the parent never match, and a cross-partition row move
(delete from one leaf, insert into another) strands the old document.
The engine's preflight detects partitions in a publication without
`publish_via_partition_root` and fails with this exact fix.

## Large values (TOAST)

Postgres omits unchanged TOASTed columns (large text/jsonb values) from
UPDATE images. Since flat sync full-replaces documents, the engine
detects these partial images and transparently re-reads the complete
row on a side connection before shipping the event — large fields never
silently vanish from destination documents. The refetch is visible as
`vs_pg_toast_refetch_total`; if a table updates TOASTed rows heavily,
`ALTER TABLE t REPLICA IDENTITY FULL` avoids the extra reads by putting
full images on the WAL.
...
- `on_missing` controls what is embedded while a related row is absent:
  `null` (default) or `empty_object`.
...
management. See [Database TLS and trust](/guides/database-tls) for the complete
decision guide.
...
With `VS_PG_BOOTSTRAP_MODE=snapshot` and no existing slot, the engine
...
the engine fingerprints the spec; if it changed, it drops the slot,
...
The engine notices on the next `RELATION` message and emits a structured
...
2. Restart with `VS_PG_AUTO_RESYNC_ON_YAML_CHANGE=true` (or
   `VS_PG_FORCE_RESYNC=true`). The fingerprint changes, so the engine drops
...
`POST /admin/resync` re-scans the tables without a restart when
[`VS_ADMIN_LISTEN`](/reference/engine-env) is configured.
...
configuration needed. (`REPLICA IDENTITY FULL` is **not** required — this
applies to flat deletes, memory-mode joins, and SQL-mode reverse-lookup child
deletes alike.)
...
was inserted and deleted entirely while the engine was down and didn't
