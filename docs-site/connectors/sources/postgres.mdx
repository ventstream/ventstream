...
Use strict TLS for a managed or remote database:

```bash
VS_PG_TLS_MODE=verify_full
VS_PG_TLS_CA_FILE=/run/secrets/postgres-ca.pem
```

The CA file is optional when the certificate chains to a public root. Strict
mode applies to logical replication, snapshots, related-row fetching, SQL
recomposition, reconciliation, and replication-slot management.
