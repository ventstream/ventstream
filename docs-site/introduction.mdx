...
The capture workload doubles as a **zero-downtime migration tool**: the
snapshot bootstrap moves existing data in bulk through the same code
path that then tails live changes, so a store can be populated, kept in
sync, and cut over without a maintenance window — see the
[migration guide](/guides/zero-downtime-migration).
...
**Migrations demand windows.** Bulk-copy scripts race the writes that
happen while they run, so moves get scheduled into maintenance windows —
or worse, skipped.
...
<CardGroup cols={2}>
  ...
  <Card title="Zero-downtime migration" icon="truck-fast" href="/guides/zero-downtime-migration">
    Bootstrap existing data, tail changes, and cut over without a window.
  </Card>
