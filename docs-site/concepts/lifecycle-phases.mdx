...
| State | Meaning |
| --- | --- |
| `running` | The engine runs the pipeline with the selected configuration revision |
...
Locally, the engine's own telemetry uses a finer phase vocabulary —
`starting`, `bootstrapping`, `tailing`, `erroring`, `stopped`, `paused`,
`drained`. The steady tail state (`tailing`) is what Cloud reports as
`running`.
