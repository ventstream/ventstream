...
```mermaid
flowchart LR
    pub["Publisher<br/>(@ventstream/sdk)"] -- "publish vs.t.…" --> broker[("NATS or<br/>Redis Streams")]
    broker --> eng{{"engine"}}
...
You don't hand-build the envelope above. Publishing takes a **minimal
contract** — the `event` name and the instance `id`, plus optional opaque
`data`:

```ts
import { VentStream } from "@ventstream/sdk";

const vs = new VentStream({ servers: "nats://localhost:4222", name: "orders-api" });
await vs.connect();

await vs.publish({
  event: "orderStatusChanged",        // what happened (may be dotted)
  id: "order_123",              // the instance id
  data: { status: "active" }, // optional, opaque
});
```

That's the whole publish surface. The SDK expands it into the full
envelope and publishes to NATS. For Redis Streams, use `RedisVentStream`
with the same publish arguments; see [Realtime brokers](/concepts/realtime-brokers#publishing-to-redis).

- `event` is the event name — no id baked in.
- `id` becomes the **last** segment of the routing subject, so the event
  lands on `vs.t.<tenant>.orderStatusChanged.`**`order_123`**. That's what makes
  it individually targetable *and* wildcard-able (see
  [Subscribing](#subscribing)) — and any number of clients can share it.
- Everything else is derived or defaulted: `id`/ULID, `received_at`,
  `schema_version`, `occurred_at` (now), `metadata` (empty); `actor` is
  optional.
- `tenant` is **not** in the payload — it comes from the publisher's
  authenticated context, never caller input.

(Reference implementation: `PublishInput` in `ventstream-protocol`;
`into_event(tenant)` performs the expansion. The TS SDK mirrors it.)

Any broker client works — the SDK is a convenience, not a requirement. NATS
publishes the envelope on the anchored subject. Redis appends `subject` and
`event` fields to the tenant stream defined in the
[Redis wire contract](/concepts/realtime-brokers#redis-wire-contract).
...
<Tabs>
  ...
  <Tab title="Raw WebSocket">
    ...
    The `@ventstream/client` package wraps this with auto-reconnect and
    subscription replay. Endpoint: `ws://localhost:4040/ws`.
  </Tab>
</Tabs>
...
| `$event.entityId` | envelope `entity_id` | (it's the `id` you publish — automatic) |
| `$event.occurredAt` | envelope `occurred_at` | set automatically by the SDK |
...
else is envelope-supplied. Same contract, either transport:
...
<Tabs>
  <Tab title="SDK">
    ...
    await vs.publish({
      ...
      id: "order_1",                  // → entityId, and the subject's last segment
      data: { status: "confirmed" },  // because a field maps $data.status
    });
    ```

    The SDK is sugar — it mints the ULID `id`, stamps the timestamps +
    `schema_version`, and composes the subject. You supply only
    `event` + `id` + the `data` the subscription reads.
  </Tab>

  <Tab title="Raw NATS">
    Any NATS client works — but you build the **full envelope** yourself
    and publish it to the composed subject. The SDK normally fills the
    boilerplate; raw, you provide it all:

    ```jsonc
    // subject: vs.t.acme.orderStatusChanged.order_1
    {
      // ── boilerplate the SDK normally generates for you ──
      "id": "01ARZ3NDEKTSV4RRFFQ69G5FAV",   // a fresh 26-char ULID per event (mint one)
      "received_at": "2026-01-01T00:00:00Z", // = now()
      "schema_version": 2,                    // must equal the current version

      // ── what you actually provide ──
      "event": "orderStatusChanged",          // the event name
      "tenant": "acme",                       // your tenant scope
      "entity_id": "order_1",                 // the instance id  ($event.entityId)
      "occurred_at": "2026-01-01T00:00:00Z",  // business time     ($event.occurredAt)
      "data": { "status": "confirmed" }       // the fields subs read ($data.*)
    }
    ```

    A bad `id`/`schema_version` or a subject that doesn't match the
    envelope is **dropped** at decode.
  </Tab>
</Tabs>
...
connection close; the SDK's jittered exponential backoff (capped at 30 s)
...
4-pod fleet — see [Kubernetes deploy](/deploy/kubernetes).
...
| `VS_GRAPHQL_SUBSCRIPTIONS` | unset | path to the typed-subscriptions manifest |
...
tenant is trusted from the handshake / connection-init. JWT validation is
planned — don't expose the gateways to untrusted clients without a proxy
that enforces auth.
</Warning>
