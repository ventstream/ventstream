...
Green is the data path — CDC from source connectors through projection and
ACK-gated delivery, and the independent broker-to-WebSocket/GraphQL fan-out.
Slate is the control path: a single outbound mTLS stream carrying desired
state, never data. Neither data path enters VentStream Cloud, and the same
engine binary runs fully standalone without a control-plane dependency.
