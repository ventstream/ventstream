...
<img
  ...
  alt="VentStream architecture showing the optional Fleet management plane above a customer-hosted data plane. Fleet administers managed agents over outbound mTLS while CDC records and realtime NATS events flow only through the VentStream engine inside the workload environment."
...
independent NATS-to-WebSocket/GraphQL delivery path. Neither data path enters
...
- `ws` and `graphql` consume NATS subjects and serve realtime clients.

Connector credentials, source data, destination documents, NATS messages, and
