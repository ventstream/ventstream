...
Green is the engine and its capabilities; blue is everything you own —
sources, targets, brokers, and clients. The only line between the planes is
a single outbound mTLS stream carrying desired state, never data: records,
documents, and events stay inside your environment. Remove the agent key
and the same binary runs fully standalone without a control-plane
dependency.
