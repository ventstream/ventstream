...
Neo4j 5.17+ Enterprise CDC via the `db.cdc.query` procedures. The
engine polls the change feed on an interval and persists an opaque
cursor string. Node and relationship creates/updates/deletes all
arrive as change records.
