...
| `VS_ROLES` | `cdc` (and/or `ws`, `graphql`), or `mcp` (which must run alone). The CDC role is the pipeline. |
...
An engine started with an agent key runs managed: it connects outbound to the
gateway, fetches its selected configuration revision, reports status, and
executes operations. Without a key, no code path opens a connection to the
platform. See [Managed mode](/concepts/managed-mode).

| Variable | Default | Meaning |
|---|---|---|
| `VS_AGENT_KEY` | unset | Agent key (`vsa1.` prefix) minted from the dashboard or CLI. Its presence switches the engine to managed mode. Must not be combined with a local `VS_ENGINE_CONFIG` pipeline config — a managed engine's pipeline config comes from the control plane. |
| `VS_MANAGED_STATE_DIR` | `/var/lib/ventstream/managed` | Absolute directory for persisted managed identity and configuration state. |
| `VS_MANAGED_GATEWAY_URL` | `https://gateway.ventstream.dev:8445` | Control-stream gateway. Must use `https://` or `wss://`. |
| `VS_MANAGED_ENROLLMENT_URL` | `https://gateway.ventstream.dev:8444` | Enrollment endpoint. Must use `https://` or `wss://`. |

## Cloud-managed configuration

`VS_FLEET_APPLIED_CONFIG_PATH` is set by the engine's fleet client in managed mode.
...
Cloud-managed config is for topology, projection, and runtime-safe settings. Keep
...
| `VS_PG_BOOTSTRAP_CHUNK_SIZE` | Rows per snapshot chunk (default 10000; 5000 in SQL denormalize mode, where the chunk also bounds each denormalizing bootstrap query). |
...
| `VS_NEO4J_TRUST_CERT_FILE` | Older alias for `VS_NEO4J_TLS_CA_FILE`; setting both is a startup error. |
...
| `VS_MYSQL_BOOTSTRAP_CHUNK_SIZE` | Rows per keyset-paginated snapshot batch (default 1000; 5000 in SQL denormalize mode, where the chunk also bounds each denormalizing bootstrap query). |
...
| `VS_JOINS_YAML` | Optional denormalization join spec (same format as Postgres). When set, related tables are embedded into one document and `VS_JOINS_STATE_DIR` persists join state. See the [MySQL connector](/connectors/sources/mysql#denormalize-modes-in-memory-vs-sql). |
...
| `VS_MYSQL_RECOMPOSE_CHUNK` | Maximum primary keys in one SQL recomposition query (default 256). Canonical config: `source.mysql.recompose_chunk`. |
| `VS_MYSQL_RECOMPOSE_CONCURRENCY` | Maximum concurrent SQL recomposition queries (default 4). Canonical config: `source.mysql.recompose_concurrency`. |
...
| Variable | Default | Meaning |
|---|---|---|
| `VS_DISPATCH_PARALLEL_BULKS` | `4` | Concurrent OpenSearch bulk requests. |
| `VS_DISPATCH_MAX_EVENTS` | `2000` | Max events per bulk batch. |
| `VS_DISPATCH_MAX_BATCH_BYTES` | `4194304` | Max bulk body size (4 MiB). |
| `VS_DISPATCH_FLUSH_MS` | `500` | Max time a batch waits before flushing. |
| `VS_BUS_CAPACITY` | `1024` | Internal event bus capacity. The default is auto-raised to hold one dispatch batch (capped at 32768); an explicit value is honored but caps sink batches at the bus size. |
