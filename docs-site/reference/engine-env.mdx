...
| `VS_SINK` | Sink kind. The current supported value and default is `opensearch`. |
...
PostgreSQL, MySQL, MongoDB, Neo4j, and OpenSearch accept the same optional
transport block beneath their connector settings:
...
`verify_full` checks both the certificate chain and hostname. Omit `ca_file`
when the certificate uses a public root. Existing configurations keep their
current URI or connector behavior when `tls` is omitted. `disabled` is the only
other mode; weak modes that encrypt without hostname verification are rejected.
...
| `VS_PG_TLS_CA_FILE` | Optional PEM CA bundle for a private CA, including cloud database CA bundles. |
...
| `VS_MYSQL_TLS_CA_FILE` | Optional PEM CA bundle for a private CA or managed-database CA. |
