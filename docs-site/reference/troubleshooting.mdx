...
description: "Diagnose engine, VentStream Cloud, authentication, agent-key, and local development failures."
icon: "wrench"
---
...
| Bootstrap skipped on restart | Retained source position means the connector resumes instead of re-scanning. Use the VentStream Cloud rebootstrap operation for supported local state. PostgreSQL drops its slot; Neo4j, MongoDB, and MySQL remove their local cursor; Kafka consumer-group offsets must be reset through Kafka administration. |
| Cloud reports an unhealthy or stale instance but the engine has no current error | The control stream or process stopped reporting. Inspect `agents status`, engine logs, and the pod restart count. |
...
| Configuration delivery waits with no agent | Confirm the deployment is connected with `ventstreamctl agents status`. The pending operation is delivered after a matching agent connects, subject to its expiry. |
| Managed engine exits at startup with an agent-key error | The `vsa1.` key was revoked, mistyped, or belongs to another deployment, or the control plane is unreachable — the engine fails closed rather than falling back to local config. Mint a fresh key (`ventstreamctl agents key create <deployment> --pipeline <pipeline>`; it revokes the previous one) and update the secret. See [Managed mode](/concepts/managed-mode). |
| Agent has a key but cannot open the control stream | Check outbound gateway reachability, system time, certificate validity, and key revocation. For private gateway trust, also verify the configured CA Secret; public Cloud gateways use WebPKI trust. An engine that lost its persisted identity state (pod rescheduled without its volume) re-binds a fresh identity from the key automatically. |
