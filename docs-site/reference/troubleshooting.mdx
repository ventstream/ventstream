...
| Symptom | Cause / fix |
|---|---|
| Agent remains `starting`, then reports `degraded` or `failed` | Check connector credentials, TLS configuration, source CDC prerequisites, and network policy. Inspect engine logs and `ventstreamctl agents status`. |
