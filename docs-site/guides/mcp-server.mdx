...
specs:
  joins: joins.yaml
runtime:
  health_listen: 0.0.0.0:4043     # /readyz for readiness probes
  mcp:
    listen: 0.0.0.0:8790
...
**Fleet-managed:** ship the same YAML as a configuration revision with
`ventstreamctl pipelines configurations create <pipeline> --engine-config …`,
then `select` it — the control plane validates it and delivers it to the
deployment like any pipeline config.
The managed-agent chart supports `workload.roles: mcp` (under
`kind: realtime`) with `service.mcpPort` exposing the listener, and
`ventstreamctl mcp keys generate --name <agent>` mints tokens, printing the token once
...
`ventstream mcp generate-token --hash` is the engine-local equivalent of
`ventstreamctl mcp keys generate --name <agent>` — both mint the token and
hash it locally, without touching the control plane; use whichever binary
is at hand.
