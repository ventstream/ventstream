...
description: "Stream typed GraphQL subscriptions over NATS — live, in the browser, zero code."
---

VentStream's real-time gateways turn NATS events into **typed GraphQL
subscriptions**. This demo brings up NATS + the engine (in `ws,graphql`
mode) and lets you watch events arrive live in the in-browser **GraphiQL**
playground — nothing to install, no client code to write.
...
```json
{ "id": "order_1", "status": "confirmed", "changedAt": "2026-01-01T00:00:00Z" }
```

The result field names differ from the payload because the SDL maps them with `@source`: `id ← $event.entityId`, `status ← $data.status`, `changedAt ← $event.occurredAt` (see [`subscriptions.graphql`](https://github.com/bashiru98/ventstream/blob/main/demo/realtime/subscriptions.graphql) and [Real-time subscriptions](/concepts/real-time-subscriptions)).
...
GraphiQL ticks once per second — `status` flipping `paid` → `shipped`,
`changedAt` advancing. That's your live subscription stream. (`st`, not
`status` — `status` is a read-only variable in zsh.)
