...
| --- | --- | --- | --- | --- |
| Standalone | `ventstream` is PID 1, no agent key | ConfigMap and Secret | Helm/Kubernetes restart and rollout | None |
| Cloud-managed | The same `ventstream` binary is PID 1; its built-in fleet client re-executes the binary as the pipeline child | Immutable Cloud revision plus workload-local Secret | Dashboard and `ventstreamctl` | VentStream Cloud |
