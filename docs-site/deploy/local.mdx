...
```graphql
subscription {
  orderStatusChanged(orderId: "order_1") { id status changedAt cursor }
}
```
...
GraphiQL updates live. The typed subscription exposes `cursor` (the resume
