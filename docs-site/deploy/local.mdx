...
Install the release binary without a Rust toolchain once the open-core release
assets are public:
...
<Note>
The repository and its release assets are currently private. For a working
installation today, use the [authenticated portable
install](/deploy/native-binary#authenticated-portable-install). It places
`ventstream`, `ventstream.yaml`, and the example configuration together under
`~/ventstream` and requires no Rust toolchain.
</Note>
...
A standalone engine needs **no control plane**. Do not set the legacy
`VS_CONTROL_PLANE_URL` or `VS_CONTROL_PLANE_KEY` telemetry variables. Lifecycle
is restart-based: stop the process to pause, and it resumes from its retained
cursor on restart.
