...
`ventstreamctl` binaries are published in the public
[VentStream releases repository](https://github.com/ventstream/ventstream-releases).
Downloading and installing the CLI does not require a GitHub account, token, or
access to the private Fleet source repository.
...
[public verification guide](https://github.com/ventstream/ventstream-releases/blob/main/docs/verification.md).

The CLI executable is public, but it still requires a reachable Fleet control
plane and an authorized user account to administer managed engines. A standalone
engine does not require `ventstreamctl`.

## Authenticate

Create and verify a built-in account or use an operator-configured OIDC device
flow. The complete account lifecycle, secure password input, recovery, refresh,
and logout commands are covered in [Authentication](/fleet/authentication).
...
List the organizations visible to the authenticated principal and select one:
