...
`ventstreamctl` is distributed with VentStream Fleet as a signed,
platform-specific binary. It is not built from this open-core engine
repository. Install the archive matching the Fleet release and verify its
checksum and provenance using the release instructions supplied with that
version:
...
Place the verified binary on `PATH`. A standalone engine does not require
`ventstreamctl`.
