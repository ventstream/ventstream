...
The managed engine stages files atomically in a private configuration
directory, then loads the staged `ventstream.yaml` and starts the pipeline.
...
After a restart, the pipeline remains stopped until the engine's fleet client
has acquired a current control session and converged the deployment.
...
customer's secret manager. VentStream Cloud validates and distributes
configuration without becoming a credential vault or data-plane dependency.
...
VentStream Cloud stores the references, TLS file paths, and key-routing policy. The
deployment provides the endpoint, credentials, and mounted trust material to
the managed engine. The engine supplies `VS_FLEET_DEPLOYMENT_ID`, which is the
...
VentStream Cloud validates the topology shape before activation. The engine resolves the
references in the workload environment, verifies the discovered primary or
...
routed targets. VentStream Cloud and the engine reject destructive Redis drain or
