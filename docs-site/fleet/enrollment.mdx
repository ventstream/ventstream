...
3. Put the returned token in a short-lived Kubernetes Secret or equivalent secret
   injection mechanism.
4. Start the managed-agent enrollment init container with the gateway server CA.
...
The current managed-agent chart continues to reference the consumed enrollment
Secret object during later pod starts. Keep that object present while
`fleet.enrollment.enabled=true`; deleting it prevents the init container from
starting even though identity state is already on the PVC. The retained token is
no longer redeemable. State-only chart bootstrap without this Secret is not yet
supported.
...
replica; replicated realtime requires a Fleet-compatible pre-provisioned identity
